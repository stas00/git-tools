...
{me} auth user repo_user repo_name new_branch_name

parameters:

  auth:      ssh or https (use ssh if your github account is setup to use ssh
  user:      your github username
  repo_user: repository name to fork/use
  repo_name: repository name to fork/use
  branch:    name of the branch to create

example:

  {me} ssh my-github-username fastai fastai new-feature-branch
...
def main():
    args = parse_args()
    was_already_forked = fork_if_needed(args.url, args.user_name, args.repo_user, args.repo_name)
...
def parse_args():
    ...
    parser.add_argument(
        'repo_user',
        metavar='REPO',
        help='repository username to fork/use'
    )
    parser.add_argument(
        'repo_name',
        metavar='REPO',
    ...
    args.orig_repo_url = f'{args.prefix}{args.repo_user}/{args.repo_name}.git'
...
def fork_if_needed(url, user, repo_user, repo_name):
    ...
    if run_and_check_success(f'git ls-remote {url} >/dev/null 2>&1'):
        ...
        run(f"curl -u {user} https://api.github.com/repos/{repo_user}/{repo_name}/forks -d ''")
        return False
...
def sync_with_master(args):
    ...
    user, repo_user, repo_name = args.user_name, args.repo_user, args.repo_name
    print(f'\n\n*** Syncing {user}/master with {repo_user}/master')
    ...
    if not ok:
        run('git merge --abort')
        print(dedent(f'''
            Error: {user}/{repo}/master can't be merged with {repo_user}/{repo_name}/master.
            You must merge manually, or reset {user}/{repo_name}/master.
